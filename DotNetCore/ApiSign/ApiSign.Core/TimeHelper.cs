...
namespace ApiSign.Core
{
    ...
    public class TimeHelper
    {
        ...
        public static DateTime dtUnix2DateTime(long dtUnix)
        {
            ...
            if (dtStr.Length == 10)
            {
                time = DateTimeOffset.FromUnixTimeSeconds(dtUnix).DateTime.ToLocalTime();
            }
            else if (dtStr.Length == 13)
            {
                time = DateTimeOffset.FromUnixTimeMilliseconds(dtUnix).DateTime.ToLocalTime();
            }
            else { }
            return time;
        ...
        public static long dtDateTime2Unix(DateTime time, int len = 10)
        {
            ...
            if (len == 10)
            {
                result = (int)dateTimeOffset.ToUnixTimeSeconds();
            }
            else {
                result = dateTimeOffset.ToUnixTimeMilliseconds();
            }
            return result;
        }
        ...
        public static long dtDateTime2Unix(int len = 10)
        {
            ...
            if (len == 10)
            {
                result = (int)DateTimeOffset.Now.ToUnixTimeSeconds();
            }
            else {
                result = DateTimeOffset.Now.ToUnixTimeMilliseconds();
            }
            return result;
        }
        #endregion
        ...
        /// <param name="secTime">第二个时间</param>
